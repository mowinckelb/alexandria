...
- **One (strong) model.** This run used a frontier-class agent that faithfully executed `block.md` — deep reads, web search, no shortcuts. A lead onboarding on a weaker model or a different harness (Cursor/Codex/Factory) could underload, skip the web, read shallow, or fabricate more. Same `block.md`, different intelligence — unverified. Re-run the onboarding step on the model tiers real leads actually use.
