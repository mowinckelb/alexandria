...
export const CONFIDENTIAL_CONTENT = {
  businessModel: {
    ...
    content: `**Sovereignty — mass-market entry.** Tool Group 1 only. Passive extraction, Constitution building, Vault sovereignty. The user adds the connector, their cognitive data accumulates in portable files they own. No philosophy required. No engagement with the Library. Just freedom insurance. Less than one coffee, for freedom insurance. The audience is everyone who uses AI. And the funnel is structurally wide: Claude is the only platform where a normal person can add a custom MCP connector in under a minute, on any plan including free. ChatGPT requires Developer Mode and paid business/enterprise plans. Gemini requires an enterprise allowlist. Every Claude user — including free tier — is a potential Sovereignty Author on day one. Two prerequisites: you use Claude, and you have cloud storage. That covers essentially the entire target market. Sovereignty is the top of the acquisition funnel.
...
Active taste extraction — and development. Taste reveals itself through thousands of micro-decisions. Passive observation gives a rough sketch. Active extraction gives a portrait. But it is not just the Editor — all three functions compound into it. The Editor extracts through Socratic questioning. Mercury refines through amplification — pushing your thinking reveals how you actually think. The Publisher refines through creation — every piece you ship is a taste decision made concrete. And the process does not just capture who you are — it develops you. The better you get, the richer the signal. The richer the signal, the deeper the Constitution. The labs build better servants. Alexandria builds a better mirror — and the mirror makes you worth reflecting.
...
**PLM is unproven.** A Personal Language Model (PLM) is a fine-tuned AI model trained on one person — not just what the Author thinks (which the Constitution already captures as structured text) but how they think: their reasoning patterns, cognitive reflexes, communication style, and taste. Where the Constitution tells the AI what the Author believes, a PLM would make the AI think the way the Author thinks. The difference matters for out-of-distribution queries — things the Author has never explicitly considered but would reason about in a characteristic way. If it works, the PLM is the highest-fidelity digital mind: a version of the Author that can operate, represent, and create autonomously. The Constitution built today is the training dataset for the eventual PLM — every investment in Constitution quality pays forward.

This is Alexandria's horizon ambition, not its current product. The technology is not ready: base models do not yet support continual learning that compounds across model generations, and per-model fine-tuning creates lock-in that contradicts sovereignty. When continual learning becomes model-agnostic — or when switching costs drop to near zero — Alexandria goes. Not a moment before. Until then, the Constitution is the product, and the PLM is the option the sovereign architecture preserves. If it never works, the Constitution-based product still stands on its own.
