...
4. Give Alexandria access to that file. Alexandria always has the file. The Author retains rights and control — can set constraints on who sees it, how it is used, whether it is public or private, whether it requires payment or approval. But Alexandria has it. This is the publisher relationship: the Author gives the manuscript to the publisher. The publisher handles distribution. Without access, the file is a private document on the Author's machine — not a contribution to the network.
