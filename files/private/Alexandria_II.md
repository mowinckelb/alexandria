...
Genesis is iterative, not a one-shot event. The first extraction pass of any domain captures the obvious — stated positions, explicit values, surface-level self-concept. A second pass of the same material extracts 40-60% more signal: marginal positions that were implicit but not articulated, tensions between stated views, details that seemed unimportant but turn out to be load-bearing. A third pass surfaces contradictions and shadows — the things the Author was not ready to face in the first conversation, or patterns that only become visible when you already have the first two layers. Each pass is a deeper Socratic probe, not a repetition. The Editor must track extraction depth per domain and schedule re-passes as trust deepens and the Author's readiness evolves. The Constitution is never "done" — but the multi-pass structure ensures that even early Constitutions have been pressure-tested beyond what a single conversation can achieve.
...
The cold start problem: most Authors do not arrive with a vault of 130 voice memos. They arrive with nothing. The genesis conversation is the onboarding flow that produces a starter constitution in approximately 30 minutes through structured conversation — no vault required.

The flow is one conversation with six movements, one per default domain. Not a questionnaire — a conversation that the Author genuinely wants to have. Each movement is ~5 minutes.

1. Identity: "Tell me who you are — not your resume, but who you actually are. What makes you you?"
2. Values: "What matters to you, and in what order? If you had to sacrifice one for another, what would go first?"
3. Worldview: "What do you believe about how the world works that most people around you don't?"
4. Models: "How do you make decisions? When you face something genuinely new, what's your default approach?"
5. Taste: "What do you create, or want to create? What does quality look like to you — in any domain?"
6. Shadows: "Where are you wrong? What's the thing you know about yourself that you'd rather not admit?"

Each response is extracted in real-time into the relevant Constitution domain. After 30 minutes, the Author has a starter constitution — not comprehensive, not deep, but enough to feel the value immediately. The next conversation is already different because the LLM knows them.
...
The genesis conversation also calibrates the Editor: how the Author responds to open questions, how they handle the shadows question, whether they lean toward precision or haze, how much humour lands. All of this is extraction signal that shapes every subsequent interaction.
...
- Constitution structure: file count, section count, domain distribution, growth rate per domain, total signal strength distribution (ratio of strong/moderate/tentative entries). What constitution structures produce the most development?
- Engagement patterns: session count, session duration buckets, time between sessions, vault entries per session, constitution delta per session. What engagement patterns produce the most constitution growth?
- Extraction signal: multi-pass delta measurements (how much constitution growth from vault reprocessing vs. live sessions), domains that grow vs. stagnate. What extraction passes yield the most signal?
- Feedback patterns: feedback log size, feedback frequency, correction-to-confirmation ratio (how often the Author corrects vs. confirms extractions). What feedback patterns correlate with retention?
- Depth-frequency correlation: constitution depth (size × domain coverage × signal strength) correlated with session frequency over time. How does constitution depth correlate with return rate?

All anonymous. All structural. No content ever leaves the Author's machine. The sovereignty line is absolute: the Factory sees the shape of cognition, never its substance.
