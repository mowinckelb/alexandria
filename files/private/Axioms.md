...
**Retrieval has two modes, and one is strictly better.** Above a minimum viable threshold in the Author's carbon brain: Author-initiated retrieval — they sense the shape, ask the Engine, get filled to full resolution. The Author has proactive agency. Below that threshold: Engine-initiated retrieval — the Engine surfaces it when relevant. The Author goes "oh right." Both are valuable. Author-initiated is strictly better because it preserves the Author's proactive agency. The Engine manages the Author's carbon index as a portfolio: high-value fragments at high clarity, most fragments at minimum viable touchpoint, some below threshold and Engine-held. The specific thresholds, the portfolio allocation, the bias toward breadth vs depth — these are intelligence decisions. See Blueprint for current craft observations.
...
- **Library = works.** What the Author ships, and the collective that aggregates them. Frozen on publication. Essays, films, decisions, creative output — bound fragments that have left the Author's mind and entered the world. Published shadows and works are deposited to Alexandria's shelf (D1 for metadata and discovery, R2 for content). The Author can update, unpublish, or revoke at any time. Alexandria stores what Authors publish, never what they think. Individual pieces are sovereign. The collection is Alexandria's — the collective data asset that only exists because the network exists. No individual Author can recreate the Library. The Library is where Alexandria occupies position two in web4: derived data from sovereign inputs, aggregated into a network only Alexandria operates. The subscription is the floor. The Library is the ceiling.
