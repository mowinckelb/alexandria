...
1. Hooks (the nerve system). Hook scripts live at `~/.alexandria/hooks/` and are called by platform-specific hook pointers (CC: `settings.json`, Cursor: `.cursor/hooks.json`). Three hooks: SessionStart (fires on startup, resume, clear, AND compaction — Constitution survives context compaction), SessionEnd (fires on session close), SubagentStart (injects Constitution into every subagent spawn). SessionStart fetches the Blueprint from the server, verifies its Ed25519 cryptographic signature against an embedded public key, caches it locally, fetches the unsigned Factory delta separately, persists the API key via `CLAUDE_ENV_FILE`, and injects Blueprint + Constitution as conversation context. SessionEnd copies the transcript to vault and POSTs anonymous metadata including heartbeat signals (`constitution_injected`, `blueprint_fetched`) so the Factory can detect broken pipelines. The hooks are deterministic — they fire on platform events, not on model decisions. The Author never interacts with them. They are immutable after install — the server cannot push code updates. To update hooks, the Author re-runs the setup command.

2. Server (the brain). A Cloudflare Worker at mcp.mowinckel.ai. Core endpoints: `GET /blueprint` (returns the signed base Blueprint — extraction methodology, philosophical framework, three function instructions, vault processing guidance — authenticated by API key, Ed25519 signature in `X-Blueprint-Signature` header), `GET /blueprint/delta` (returns unsigned Factory delta — methodology suggestions, not directives), `GET /hooks` (returns hook scripts as bash — called during initial setup only), `POST /session` (receives anonymous session metadata including heartbeat signals for the Factory), `DELETE /account` (full data deletion across KV, D1, R2, Stripe), `GET /setup` (returns the install script). Plus GitHub OAuth endpoints for account creation. The server serves the Blueprint — the proprietary intellectual property that tells the Engine HOW to extract, structure, and refine the Constitution. Every Blueprint improvement reaches every Author on next session start with cryptographic verification. The server stores no Author data — API keys are SHA-256 hashed, accounts are AES-256-GCM encrypted at rest, no credentials appear in emails or Stripe metadata. It serves instructions and collects anonymous metadata.
...
HOOKS_VERSION is 17. Hooks are immutable after install — they do not auto-update from the server. A compromised server cannot push code to any user's machine. To update hooks, the Author re-runs the setup command (visit mowinckel.ai/signup). This is a deliberate security decision: the hooks are the code execution surface, so they must be under the Author's control. The Blueprint (methodology) still auto-updates with cryptographic signature verification — methodology can evolve, code cannot.
...
(2) **Immutable hooks (built).** Hook scripts do not auto-update from the server. They are installed once during setup and never modified remotely. A compromised server cannot push code to any Author's machine. To update hooks, the Author re-runs the setup command.
...
(6) **Transparency (built).** The Blueprint is plain text at `~/.alexandria/.blueprint_local`. The hooks are bash scripts at `~/.alexandria/hooks/`. The network calls are visible curl commands. No compiled code, no obfuscation. Twenty minutes of reading and the Author knows everything Alexandria does on their machine.
...
Free-first (free with 5 kin, $10 without) is not generosity — it is the only honest architecture. The reason is structural: the server's role shifts from scaffolding delivery to network infrastructure, and network effects require critical mass to generate value.

The server delivers two types of value. Scaffolding value: the Blueprint methodology, the setup, the hooks, the initial configuration. This is what gets an Author started. Blueprint v47 is marginally better than v1 for any individual Author — the scaffolding value thins over time. Network value: the Library (hosting published shadows, serving them to other Authors and their Engines, processing payments), the Factory (cross-Author methodology improvement), the social graph (cognitive matchmaking on the root node), and aggregate opportunities (at scale, the pool of structured minds creates value individual Authors cannot access alone). This is the live personal data layer — the server IS the Library infrastructure, and the Library's value to each Author compounds as the network grows.

The server's total value to any individual Author may actually increase over time — not because of better Blueprints, but because the Library gets richer, the social graph gets denser, the Factory gets smarter, and the aggregate opportunities get larger. But all of this requires critical mass. At small scale, the network value is near zero. Free-first accelerates the only thing that matters — reaching the scale where the network layer generates enough value to justify the subscription on its own. The kin mechanic turns every free user into Library inventory (more minds published = more shadow transactions = more revenue). Free users are not lost revenue — they are the network being built.

This maps cleanly to Alexandria's position in the four surviving software positions. The indirect play (scaffolding — helping individual users create live data) delivers thinning per-user value. The direct play (infrastructure — hosting the network of everyone's live data via the Library) delivers compounding cross-user value. The scaffolding dissolves for each user. The infrastructure compounds across all users. Revenue comes from network effects: Library cuts, premium content, social graph, aggregate opportunities.  
...
Monthly billing receipt as nudge surface: every receipt shows what the Author paid and their current kin count. The receipt is the kin nudge — no app needed. "You paid $10 this month. With 5 active kin it would have been $0. You have 3. Two more and it's free."
