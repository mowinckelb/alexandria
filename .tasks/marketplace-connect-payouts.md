...
- **[DONE 2026-06-28] O. Author "set your price" path** — per-file price, agent-native (no UI): `PUT /file` accepts `price_cents`, stored on `protocol_files` (lazy column via `ensureFilePriceColumn`, no migration apply). Checkout uses per-file price → per-author default → $2, with the author's price as the FLOOR (buyer may tip up via `amount_cents`, never underpay — closed a pre-existing underpay leak). Reconciliation-safe (COALESCE preserves price when a re-PUT omits it; unchanged-check includes price). Price clamp ($1–$1000) is single-sourced in `db.ts` (`clampPaidAmount`). **Follow-ups — both DEFERRED 2026-06-28 (do-what's-optimal call: gold-plating a zero-user feature; revisit at the first real paid-selling author, which is the natural trigger):**
- (a) **Works pricing** — works still use the buyer $20–$200 slider, not author-set pricing, so files & works differ. The slider is a working pay-what-you-want model; unify to author-set-price-as-floor only if/when a real author needs it. Product call, not a bug.
- (b) **Agent-doc** — for "tell your AI to set the price" to work via the agent, add `price_cents` to the `PUT /file` body documented at `factory/canon/publisher.md` (the publish-mapping line, ~L50) and ship with `bash factory/ship.sh "doc: price_cents on PUT /file"`. NOT done now because: it needs the founder's offline passphrase-signing key (`~/.alexandria-signing/key`) — agent can't sign — AND an unsigned canon edit on `main` breaks every Author's payload verification, so it's a founder-signed action; and it's inert until an author actually publishes a paid file. **Trigger: first real author wants a custom price → make that one-line edit + `ship.sh`.** (The API already accepts `price_cents`; this is only the agent-instruction layer.)
