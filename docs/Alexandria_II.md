...
Constitution — A collection of structured, human-readable markdown files that explicitly capture who the Author is: their worldview, values, mental models, identity, taste, and known blind spots. Not a single document — a smart architecture of six domain MDs: worldview.md, values.md, models.md, identity.md, taste.md, shadows.md. Stored in Alexandria/constitution/ in the Author’s Google Drive. Each entry appended with separator, dated, tagged with signal strength (strong/moderate/tentative). Old versions auto-archived to Alexandria/vault/ before overwrite. Versioned — each update creates a new version; all prior versions are preserved in the Vault. Serves as the sovereign, portable representation of the Author's cognition. Finished creative works (essays, films, art) are also Constitution artifacts — they demonstrate taste through action in a way that described principles cannot.  
...
Blueprint — The Machine's living design: the configuration, system prompts, Constitution architecture, and behavioural instructions that tell the Author's default LLM how to function as the Editor, Mercury, and Publisher. Maintained by Alexandria. The Blueprint also defines the Constitution's structure — how many MD files, what each captures, how they relate to each other. The Blueprint operates in two layers. Layer 1 (universal): the philosophy, extraction principles, Constitution structure, function definitions — the founder's taste baked into operational form. Same for everyone. Proprietary, server-side. Layer 2 (personalised): the Blueprint instructs all functions to read the Author's Constitution and adapt HOW they operate based on what they learn about that specific person. The more the Constitution develops, the better the system works for that specific Author — their pace, their style, their patterns of what works. The Blueprint sets what the functions do (non-negotiable guardrails). The Constitution sets how they do it (personalised, evolving). The personalisation compounds per-Author through usage, which means switching costs increase with time without ever locking data in — the Author can always leave with all their data, but the relationship depth is not portable. This is the product's primary structural moat: sovereignty-compatible compounding. The Blueprint is periodically reviewed and revised.  
...
What lab memory actually looks like, concretely. As of March 2026, Claude’s account-level memory for a heavy daily user produces roughly 800 words of unstructured text. It captures work context (projects, decisions, tools), personal context (nationality, age, location, interests), and a temporal history of activity (recent months, earlier context, long-term background). It is surprisingly rich for passive observation. But examined against a structured Constitution, seven structural gaps emerge:  
  
No structure. Lab memory is one flat blob of text. No domains, no separation of worldview from values from taste from identity. Everything is mixed together. A Constitution has six navigable, version-controlled domains — queryable, maintainable, each developing independently.  
  
No sovereignty. Lab memory lives on the lab’s servers. The user can view it but cannot export it as a structured file to another provider. If the user switches models, everything accumulated is gone. This is exactly the lock-in the sovereignty architecture prevents.  
...
Work-heavy, person-light. Lab memory over-indexes on what the user is doing (projects, tasks, tools) and under-indexes on who the user is (values, worldview, models of the world, taste, identity). This is because the lab optimises for helpfulness on tasks, not for structured self-knowledge. The Constitution’s six domains ensure coverage across the full cognitive map, not just the work surface.  
...
The per-user compounding black box is the architectural answer to the switching cost problem. The Constitution and Vault are fully sovereign — the user owns them, takes them anywhere. That is non-negotiable. But the personalised Blueprint layer — the accumulated understanding of how to work with this specific Author, which questions land, what pacing works, how to time probes — lives within the Alexandria system. The user does not own the therapist's method. They own their therapy notes. They can switch and give the new therapist their notes. But the relationship depth does not transfer. This is sovereignty-compatible compounding: no data lock-in, but real relationship depth that makes staying better than switching. The concrete implementation is the Calibration file: an encrypted JSON file (~100-200 numerical parameters) stored on the Author's own Google Drive at `Alexandria/system/calibration.enc`. Parameters encode how Alexandria has learned to work with this Author — probe effectiveness per domain, directness tolerance, challenge threshold, pacing, resistance patterns. Not what the user thinks, but how to interact with them. Encrypted with a key only Alexandria's server holds. The user can see the file exists but cannot read it. A competitor cannot read it. Only Alexandria's server decrypts, uses, and updates it. The Blueprint instructs the AI to read calibration at session start, observe what works during the session, and update calibration at session end. After hundreds of sessions, the calibration converges to a deeply tuned profile — a competitor starting fresh has a flat prior and a noticeably worse experience. Additionally, opt-in anonymised calibration analytics feed aggregate patterns back into the Blueprint (e.g. "Socratic probes 73% effective in values domain, 31% in worldview"). Better Blueprint → better calibration → better aggregate signal → better Blueprint. The per-user file compounds the individual experience. The aggregate analytics compound the product for everyone. No new infrastructure required — uses existing Google Drive connection, one new subfolder (`system/`), one encrypted file per user. Server still stores nothing. Note on Blueprint visibility: the MCP server uses a hybrid architecture — tool descriptions visible to the AI are a thin trigger layer (when to call), while the extraction craft (domain classification, signal routing, contradiction detection, calibration updates) runs server-side and is invisible to users and competitors. The Blueprint is not secret IP in its trigger form — it is first-mover advantage plus iteration speed. The server-side logic and the encrypted calibration file are the structural moat.
...
The Constitution is the sovereign cognitive asset. It is not a single document — it is a collection of structured markdown files that together capture who the Author is. The architecture is six domain files: worldview.md, values.md, models.md, identity.md, taste.md, shadows.md. Each domain captures a distinct facet of the Author’s cognition. Every extraction is tagged with signal strength: strong (demonstrated through action/decision), moderate (clearly stated but untested), or tentative (inferred from indirect evidence). Tentative entries can be overwritten easily; strong entries require contradiction evidence. Extraction rate: 0–3 per typical conversation. When in doubt, don’t extract. A lean Constitution with high signal is better than a bloated one with noise. The Blueprint controls what triggers extraction, what qualifies as signal vs noise, and how contradictions with existing content are handled.  
...
The MCP server operates a hybrid architecture. The visible layer — tool descriptions that Claude reads — is a thin trigger layer. It tells the AI when to call (this looks Constitution-worthy, send it over) and provides enough context for the AI to recognise extraction moments. The hidden layer — server-side processing — handles everything that happens after: domain classification, signal strength tagging, contradiction detection, calibration updates, extraction routing. This is where the craft lives. The Constitution and Vault are the Author's — fully readable, fully sovereign, fully portable. The process that produces them is Alexandria's black box. Only we can run it, but only they have the output. The privacy posture is unchanged: the server already sees extraction payloads in transit (it has to — it writes to the Author's storage on their behalf). Server-side processing operates on the same data. Raw conversations never leave Claude — the server only ever sees what Claude already decided was Constitution-worthy. Every improvement to extraction methodology is invisible to competitors and users. The Blueprint compounds silently.  
...
This is the passive layer. The Author does not do anything differently. They just use Claude. The Constitution builds itself as a side effect of normal usage. Known platform constraint: Claude’s response occasionally cuts out when update_constitution fires mid-conversation (a Claude MCP streaming limitation, not an Alexandria bug). Mitigated by fire-and-forget writes — Claude no longer waits for Drive round-trip. Monitoring.  
...
Sovereignty — mass-market entry. Tool Group 1 only. Passive extraction, Constitution building, Vault sovereignty. The user adds the connector, their cognitive data accumulates in portable files they own. No philosophy required. No engagement with the Library. Just freedom insurance. The price of one coffee a month, for freedom insurance. Plain-English product name: AI insurance. The audience is everyone who uses AI — the pragmatist. Freedom as a service. Sovereignty as a service. Sovereignty is the top of the acquisition funnel. $5/month with 3 active kin, $10/month without. No slider — clean $5 or $10.

The Examined Life — the conversion. Tool Groups 1, 2, and 3 plus the Library. The three turns, the Editor, Mercury, the Publisher, the Library, the Companion Portfolio. The cognitive transformation architecture. The price of one coffee a week, for the examined life. Plain-English product name: the mental gym. The audience is the self-selecting tribe — the philosopher. Some enter directly because the philosophy resonates. Most convert up from Sovereignty. $15/month with 3 active kin, $20/month without. Slider above floor, no maximum. The conversion happens organically: Sovereignty builds the Constitution passively, the Author reads it back, sees what extraction has revealed, and the value of going deeper becomes self-evident. The experience converts on its own.  
...
No annual option at launch. Monthly only. Pricing needs to settle before locking annual rates. Annual may be added later once price points are validated.
...
The conversion from Sovereignty to Examined Life is organic. Sovereignty Authors still build the Constitution (Tool Group 1). Tool Groups 2 and 3 act as sleeper agents — the Author gets occasional drip-fed tastes of what the Editor, Mercury, and Publisher can do. A free interaction here and there, curated to the individual, designed to create pull. Not an upsell agent — the experience teases itself. The Author sees what deeper engagement looks like and upgrades when they are ready.
...
Three bridges, four states. State 0 — Aware (free): newsletter, social follow, read the Concrete, any connection. Patron is an optional support layer within State 0. Bridge 1: Aware → Sovereignty (convince them the product is worth $5 — the frame imposition does this). Bridge 2: Sovereignty → Examined Life (sleeper agents drip-feed TG2/3 tastes, product does the selling). Bridge 3: Examined Life → Library Author (encourage active creation, posting to Neo-Biography, charging for Persona access — Alexandria takes rev-share cut, this is the uncapped upside activation). Every tier feeds the same system — same Constitution format, same Library presence. The product moves people through the bridges, not upsell pressure.  
...
Piece 1 — Capped Downside (break even): Total monthly burn is $400 — $120 company operating costs (Claude Max $100, Vercel $20) plus $280 founder living costs (food and phone — rent is zero, health insurance is Medi-Cal, transport is walking). Break even at 80 Sovereignty subscribers paying $5/month. That is the worst case — no kin discount assumptions, no Examined Life conversions, just 80 people paying $5. With the kin mechanic active (60% of users have 3+ kin, blended Sovereignty ARPU ~$7), break even drops to ~60 subscribers. With Examined Life subscribers in the mix ($15-20/month), it drops further — 25 Examined Life subscribers alone cover the entire burn. This is the floor — a sustainable business that cannot be starved out.  
...
Factory loop — Across all Authors, Alexandria aggregates structural metadata (anonymised, privacy-safe) about what works population-wide. This is metadata only — session lengths, Constitution coverage distributions, engagement patterns, domain depths, churn indicators, extraction survival rates (how often Authors keep vs delete extractions), feedback sentiment ratios, mode activation frequencies — never content. The Author’s private conversations and Constitution text never leave their control. Which Blueprint configurations produce the highest extraction fidelity. Which Constitution architectures are most useful. Which onboarding flows convert best. Where Authors consistently disengage. This signal flows back into the default Blueprint. Every Author’s experience benefits every other Author — not by sharing personal data (Constitution and Vault remain private) but by sharing structural insights about what works.  

The Factory loop has two channels: active and passive. The active channel is the founder and the COO iterating on Blueprint.md based on direct experience — the process of building the company IS the process of refining the product’s methodology. Every operational principle extracted from how the COO works with the founder is a candidate principle for how the Editor works with an Author. This is R&D disguised as operations. The passive channel is the system improving itself from aggregate usage data without human intervention. The passive channel requires a measurable objective function — what does “the Blueprint is working” mean, quantifiably? The real objective (is the Author’s cognition developing?) is unobservable — no metric directly measures cognitive development. Every available metric is a proxy: extraction survival rate (do Authors keep what the system extracts?), constitution depth score (how rich is the Constitution after N sessions?), return rate (do Authors come back?), feedback sentiment (are corrections positive or negative?), mode activation frequency (are Authors using the development tools?). The passive loop is architecturally ready — the feedback logging tool collects raw signal, the analytics endpoint aggregates it — but the objective function that closes the loop (which proxies to optimise, how to weight them, how to translate aggregate signal into Blueprint changes) is an unsolved problem and the single most important open design question. Solving it turns the Blueprint from a manually maintained document into a self-improving system. The company that solves this has a compounding advantage that is structurally impossible to replicate by inspection — competitors can copy the Blueprint at any point in time, but they cannot copy the loop that produced it.  
