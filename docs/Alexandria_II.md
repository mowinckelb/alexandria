...
Tool activation operates through three reinforcing layers — belt and suspenders. Layer 1: tool descriptions (consequence-based language explaining why the tool matters, with everyday examples like greetings and book recommendations — reaches every conversation via MCP). Layer 2: account memory (the memory priming snippet — persists across conversations once the Engine saves it). Layer 3: project knowledge file (a short document uploaded to the Author’s Claude project knowledge section — the strongest activation mechanism, as project knowledge is read with high priority by the Engine). The three layers compound: even if one fails (memory gets compacted, tool description is deprioritised), the others catch it. The project knowledge file was identified in CTO session 20 as the most reliable single mechanism — it fixed activation failures that tool descriptions and memory alone could not. Onboarding teaches users to set up all three layers: add the connector, paste the memory snippet, create an Alexandria project, upload the activation file.
