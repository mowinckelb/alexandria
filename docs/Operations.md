...
- **Threshold vs maximisation games on every dimension.** Every dimension of every document is either a threshold game (reach the bar, then stop caring) or a maximisation game (push as far as possible). The dimension you are maximising is your objective function. The dimensions you are thresholding are constraints. This framework governs how honesty and fidelity interact across all documents:
  ...
  - **Investor-facing (Memo.md, Deck, Logic.pdf):** Threshold game on honesty — the threshold is high because investors are betting money and will scrutinise with hostility. Any dishonesty that gets caught invalidates everything. Build in a safety margin above the threshold. But once comfortably above the honesty threshold, the maximisation game is conversion — getting investors to say yes. That is the actual objective function. Honesty is the constraint. Conversion is the goal.
...
- Cost and break-even numbers propagate to more documents than expected. A single cost change touches: Numbers.xlsx, Finance.md, A2 (revenue model), Memo.md (body + table + reference notes), Surface.md (scroll copy), Code.md (CTO sync). Always grep for the old number across every file before assuming the change is done.
...
- Turn 2.5 complete on Railway (CTO session 15). 5 tools total (down from 10). Architecture: Philosophy → Intelligence → Verification. Server = bridge. activate_mode replaces separate editor/mercury/publisher tools. Autonomous Factory loop running daily. Monitoring dashboard live. Persistent event log at /data.
