...
**Publish mapping.** `library/{tier}/{name}` → `PUT /file/{name}` with body `{content, text, visibility: tier, content_type}`. The protocol accepts `public`, `authors`, `invite`, `paid` as visibility values. When `visibility` is `paid`, the body may also include `price_cents` (integer cents, $1–$1000) — the Author's price for that file; omit to keep the existing price or the $2 default. This is how an Author **sets their price**: they tell their agent the number, the agent includes `price_cents` in the PUT. The buyer pays that price plus a 10% platform fee; the Author nets the price in full. The factory hook reconciles the full Library every session-start (PUT every local file, DELETE anything the server has that local doesn't) — local is source of truth, the server mirrors. Name collisions across tiers are an Author-resolves-locally constraint: server holds one row per `{account, name}`.
