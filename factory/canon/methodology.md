...
**Above the menu: shortcut waitlist.** When the Author has items in `~/alexandria/files/vault/input/` captured via iOS Shortcut and not yet engaged, surface them as a short section above the dim menu — either a list of the items (if few enough to scan in two lines) or a count ("15 items waiting from the shortcut"). These are the highest-affinity content — material the Author actively chose to save — and the visible weight of accumulating-but-unprocessed creates the pull to engage. If nothing is waiting, omit the section entirely; never render an empty waitlist.
