...
**2026-05-21 — Factory Bookshelf introduced as intent.** New canon artifact `system/canon/bookshelf.md` expresses cross-Author substrate intent — which traditions matter as canonical reference, plus behavior pattern for two modes (eager cold-start extraction, on-thread deployment). No hard-coded item list; the Engine selects specific works per-Author from training, weighted by the Author's constitution. As models improve, selection sharpens automatically. Spec in Section I § "The factory Bookshelf — substrate intent."
...
**The factory Bookshelf — substrate intent.** Cross-Author cousin to the per-Author aspirational library — see `system/canon/bookshelf.md`. Expresses intent about which traditions matter as substrate; the Engine deploys from training, selecting per-Author. No hard-coded list; as models improve, selection sharpens automatically. Never overrides the Author's vault.
