...
**2026-05-21 — Factory Bookshelf introduced.** New canon artifact `system/canon/bookshelf.md`: the founder's suggestions for cross-Author substrate — cold-start deep-extract sources (Aurelius, Tao Te Ching, founder's YouTube playlist, Nunokawa coursepack) plus anchor reference picks (~22 canonical works) the Engine deploys when threads activate. Treated as suggestions, not authoritative — Engine selects per-Author and reaches beyond as fit demands. Spec in Section I § "The factory Bookshelf — substrate suggestions."
...
**The factory Bookshelf — substrate suggestions.** Cross-Author cousin to the per-Author aspirational library — see `system/canon/bookshelf.md`. The founder's suggestions for cold-start deep-extract sources and on-thread reference picks. Engine treats as starting points; selects and reaches beyond per-Author. Never overrides the Author's vault.
