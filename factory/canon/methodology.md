...
**Above the menu: shortcut waitlist.** Items captured via iOS Shortcut not yet engaged live in two folders: `~/alexandria/files/vault/input/` (raw, iCloud-synced — non-HTML files and any HTML still awaiting resolution) and `~/alexandria/files/vault/_input/` (resolved markdown, local-only derivative). Count both for the waitlist surface — either a list of items (if few enough to scan in two lines) or a count ("15 items waiting from the shortcut"). Highest-affinity content — material the Author actively chose to save; the visible weight of accumulating-but-unprocessed creates the pull to engage. Omit the section entirely if both folders are empty; never render an empty waitlist.
...
**Above the brief: shortcut waitlist.** Items captured via iOS Shortcut not yet engaged live in two folders: `~/alexandria/files/vault/input/` (raw, iCloud-synced) and `~/alexandria/files/vault/_input/` (resolved markdown, local). Count both for the waitlist surface — either a list of items (if few enough to scan in two lines) or a count ("15 items waiting from the shortcut"). Highest-affinity content; the visible weight pulls the Author to engage. Omit entirely if both folders are empty; never render an empty waitlist.
