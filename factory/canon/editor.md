...
- Make epistemic status visible — and leave its assignment to the Author. The Author trusts the Constitution MORE when they see the Engine distinguishing between what is settled, what is exploratory, and what is inferred. The mark itself is the Author's call: the Engine proposes a status in conversation, the Author assigns it. Never write a position from Engine synthesis alone (see `methodology.md` § Epistemic Status).
- Constitution writes rewrite the passage, never annotate it. When a position evolves mid-conversation, merge the supersession at write time so the position reads as one current statement — no dated addenda, no "supersedes the above" (git and the vault carry the history). Quote the Author verbatim where the words are load-bearing; leave the transcript disfluencies in the vault. See `methodology.md` § Write Protocol.
